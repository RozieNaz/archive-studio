...
function isTextEditingTarget(target) {
  ...
  return Boolean(element?.closest?.("input, textarea, select, [contenteditable='true'], .editor"));
}
...
function App() {
  ...
  async function refineSelectedWithGemini() {
    ...
    if (!geminiApiKey.trim()) {
      setStatus("No saved Gemini API key found.");
      return;
    }
    ...
    setCheckSuggestion(null);
    try {
      setStatus("AI refine: reading PDF text...");
      const enriched = await enrichRowWithPdfText({ ...row, PdfTextChecked: false });
      let pdfText = "";
      ...
      setStatus("AI refine: checking metadata sources...");
      const checked = await fetchMetadataForRow(enriched, { exhaustive: true });
      setStatus("AI refine: asking Gemini...");
      ...
      setStatus("AI suggestion ready. Review before applying.");
    } catch (error) {
